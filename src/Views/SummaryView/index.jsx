...
import __grammar_html from "./nguphap";
// console.log(__grammar_html);
var template = __html.map((item)=> {return({__html: item} )});
var grammar_template = __grammar_html.map((item)=> {return({__html: item} )});
// console.log(template);
import { Image, Col, Row, Container, Pagination } from "react-bootstrap";
...
function SummaryView() {
  ...
  const useHeadingsData = () => {
    ...
    React.useEffect(() => {
      const headingElements = Array.from(
        document.querySelectorAll("main .grammar")
      );
  ...
  const useIntersectionObserver = (setActiveId) => {
    ...
    React.useEffect(() => {
      ...
      const headingElements = Array.from(document.querySelectorAll(".grammar, h3"));
  ...
  return (
    <Layout title="Tổng hợp kiến thức thi Toeic">
      ...
      <h1>Tổng hợp kiến thức cần thiết để thi Toeic</h1>
      <Tabs
      defaultActiveKey="Ngữ Pháp"
      id="uncontrolled-tab-example"
      className="mb-3"
    >
      
      <Tab eventKey="Ngữ Pháp" title="Ngữ Pháp">
        <Row>
          <Col md="auto">
            <main className="summary">
              <div className="grammar">
              <h2 className="grammar" id="Tenses-header">I. Tenses - thì tiếng Anh</h2>
              <p>
              ...
              />
              <div className="content" dangerouslySetInnerHTML={grammar_template[0]}></div>
              </div>
              <h2 className="grammar" id="Gerund-header">
              ...
              <div className="content" dangerouslySetInnerHTML={grammar_template[1]}></div>
              <h2 className="grammar" id="Modal-verbs-header">
              ...
              />
              <h2 className="grammar" id="Comparison-header">IV. Comparison - Câu so sánh</h2>
              <p>
              ...
              <h2 className="grammar" id="Passive-header">V. Passive Voice - Câu bị động</h2>
              <p>
              ...
              <h2 className="grammar" id="Subject-header">
                VI. Subject - Verb Agreement: Sự hòa hợp giữa chủ ngữ và động từ
              </h2>
              <p>Sự hòa hợp giữa chủ ngữ và động từ (Subject-Verb Agreement) là một trong những chủ đề ngữ pháp tiếng anh quan trọng, thú vị nhưng đặc biệt dễ gây nhầm lẫn. Trong các kỳ thi chuẩn hóa năng lực ngôn ngữ Anh, dạng bài về hòa hợp chủ vị xuất hiện với thường xuyên và chưa bao giờ vắng mặt.

Về nguyên tắc chung, chủ ngữ số ít đi cùng động từ số ít (động từ thêm s/es) và chủ ngữ số nhiều đi cùng động từ số nhiều (động từ nguyên thể, không chia). Tuy nhiên, đôi khi động từ cần hòa hợp với chủ ngữ theo ý tưởng diễn đạt hoặc danh từ/ đại từ đứng trước theo các quy tắc nhất định. Không hệ thống một cách khoa học sẽ làm người học gặp khó khăn và dẫn đến nhầm lẫn</p>
              <h2 className="grammar" id="IF-header">VII. IF - Câu điều kiện</h2>
              <p>
              ...
              <h2 className="grammar" id="Clause-header">VIII. Clause - Mệnh đề </h2>
              <p>
              ...
              <h2 className="grammar" id="past-p2-header">
              ...
              <h2 className="grammar" id="Subjunctive-header">X. Subjunctive - Câu giả định</h2>
              <p>
              ...
              <h2 className="grammar" id="Question-header">
              ...
              <h2 className="grammar" id="Types-header">XII. Types of words - Từ loại Tiếng Anh</h2>
              <p>
      ...
      <Tab eventKey="Từ Vựng" title="Từ Vựng">
      <h2>Từ Vựng</h2>
      <div className="content" dangerouslySetInnerHTML={template[activeTopic]}></div>
      <Pagination className="d-flex justify-content-center" style={{ maxWidth: "450" }}>
                    <Pagination.First onClick={() => setActiveTopic(0)} />
                    <Pagination.Prev
                      onClick={() =>
                        setActiveTopic(Math.max(activeTopic-1,0))
                      } />
                    {activeTopic > 3 ? <Pagination.Ellipsis /> : null}
                    {activeTopic > 2 ? (
                      <Pagination.Item 
                      onClick={() =>
                        setActiveTopic(activeTopic - 2)
                      }>{activeTopic - 1}</Pagination.Item>
                    ) : null}
                    {activeTopic > 1 ? (
                      <Pagination.Item 
                      onClick={() =>
                        setActiveTopic(activeTopic - 1)
                      }>{activeTopic}</Pagination.Item>
                    ) : null}
                    <Pagination.Item active>{activeTopic +1}</Pagination.Item>
                    {activeTopic < template.length - 1 ? (
                      <Pagination.Item 
                      onClick={() =>
                        setActiveTopic(activeTopic +1)
                      }>{activeTopic + 2}</Pagination.Item>
                    ) : null}
                    {activeTopic < template.length - 2 ? (
                      <Pagination.Item 
                      onClick={() =>
                        setActiveTopic(activeTopic + 2)
                      }>{activeTopic + 3}</Pagination.Item>
                    ) : null}
                    {activeTopic < template.length - 3 ? (
                      <Pagination.Ellipsis />
                    ) : null}
                    <Pagination.Next 
                      onClick={() =>
                        setActiveTopic(Math.min(activeTopic+1,template.length-1))
                      }/>
                    <Pagination.Last
                      onClick={() =>
                        setActiveTopic(template.length - 1)
                      }
                    />
                  </Pagination>
      </Tab>
