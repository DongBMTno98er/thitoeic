  ...
  const useHeadingsData = () => {
    ...
    React.useEffect(() => {
      const headingElements = Array.from(
        document.querySelectorAll("main .grammar, main h3")
      );
        ...
        >
          ...
          <Tab eventKey="Từ Vựng" title="Từ Vựng">
            ...
            <Pagination className="d-flex justify-content-center" style={{ maxWidth: "450" }}>
              ...
              <Pagination.Prev
                onClick={() => setActiveTopic(Math.max(activeTopic - 1, 0))} />
              {activeTopic > 3 ? <Pagination.Ellipsis /> : null}
              {activeTopic > 2 ? (
                <Pagination.Item
                  onClick={() => setActiveTopic(activeTopic - 2)}>{activeTopic - 1}</Pagination.Item>
              ) : null}
              {activeTopic > 1 ? (
                <Pagination.Item
                  onClick={() => setActiveTopic(activeTopic - 1)}>{activeTopic}</Pagination.Item>
              ) : null}
              <Pagination.Item active>{activeTopic + 1}</Pagination.Item>
              {activeTopic < template.length - 1 ? (
                <Pagination.Item
                  onClick={() => setActiveTopic(activeTopic + 1)}>{activeTopic + 2}</Pagination.Item>
              ) : null}
              {activeTopic < template.length - 2 ? (
                <Pagination.Item
                  onClick={() => setActiveTopic(activeTopic + 2)}>{activeTopic + 3}</Pagination.Item>
              ) : null}
              {activeTopic < template.length - 3 ? (
                <Pagination.Ellipsis />
              ) : null}
              <Pagination.Next
                onClick={() => setActiveTopic(Math.min(activeTopic + 1, template.length - 1))} />
              <Pagination.Last
                onClick={() => setActiveTopic(template.length - 1)} />
            </Pagination>
          </Tab>
        </Tabs>
      </Container>
