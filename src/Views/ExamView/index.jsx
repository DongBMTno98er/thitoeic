...
function ExamView(props) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setDataOrder(response.filter((item) => item._id == id)[0].question.sort((a,b)=>(a.newOrder-b.newOrder)))
        ...
        <Container className="Container">
          ...
          {isStart ? (
            <>
              ...
              <Row>
                {dataProcess(data).sort((a,b)=>(a.newOrder-b.newOrder)).map((item, index) => {
                  return (
                    <div
                      key={item.id}
                      className={`topic ${
              ...
              <Row style={{ marginTop: 100 }}>
                ...
                <Col md={"auto"}>
                  <Pagination style={{ maxWidth: "450" }}>
                    ...
                    {activeTopic > 2 ? (
                      <Pagination.Item>{dataOrder[activeTopic-2].newOrder}{dataOrder[activeTopic-2].hasChild?("-"+(dataOrder[activeTopic-2].newOrder+dataOrder[activeTopic-2].childCard.length-1)):""}</Pagination.Item>
                    ) : null}
                    {activeTopic > 1 ? (
                      <Pagination.Item>{dataOrder[activeTopic- 1 ].newOrder }{dataOrder[activeTopic-1].hasChild?("-"+(dataOrder[activeTopic-1].newOrder+dataOrder[activeTopic-1].childCard.length-1)):""}</Pagination.Item>
                    ) : null}
                    <Pagination.Item active>{dataOrder[activeTopic].newOrder}{dataOrder[activeTopic].hasChild?("-"+(dataOrder[activeTopic].newOrder+dataOrder[activeTopic].childCard.length-1)):""}</Pagination.Item>
            ...
            <ResultView userData={saveAnswer.current} examData={data} scoreData = {scoreData}/>
          ) : (
            <TestInforCard />
          )}
        </Container>
